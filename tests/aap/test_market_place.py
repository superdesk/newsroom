...
def test_share_items(client, app):
    ...
    with app.mail.record_messages() as outbox:
        resp = client.post('/wire_share?type=aapX', data=json.dumps({
        ...
        assert 'http://localhost:5050/aapX?item=%s' % parse.quote(items[0]['_id']) in outbox[0].body
        assert 'http://localhost:5050/aapX?item=%s' % parse.quote(items[1]['_id']) in outbox[0].body
        assert 'Some info message' in outbox[0].body

    resp = client.get('/aapX/{}?format=json'.format(items[0]['_id']))
    data = json.loads(resp.get_data())
