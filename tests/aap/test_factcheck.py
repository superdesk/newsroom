...
def test_blueprint_registration(client):
    resp = client.get('/factcheck')
    assert resp.status_code == 200


def test_item_detail(client):
    resp = client.get('/factcheck/tag:foo')
    assert resp.status_code == 200
...
def test_item_copy(client, app):
    resp = client.post(
        '/factcheck/{}/copy'.format(items[0]['_id']),
        content_type='application/json'
    )
    assert resp.status_code == 200
...
def test_download(client, app):
    ...
    for _format in wire_formats:
        _file = download_zip_file(client, _format['format'], 'factcheck')
...
def test_notify_user_matches_for_new_item_in_history(client, app, mocker):
    ...
    app.data.insert('history', docs=[{
        'version': '1',
        '_id': 'bar',
    }], action='download', user=user, section='factcheck')
    ...
    assert len(outbox) == 1
    assert 'http://localhost:5050/factcheck?item=bar' in outbox[0].body
